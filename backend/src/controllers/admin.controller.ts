...
import { sendAdminCreatedUserEmail } from "../services/adminAccountEmail.service";
...
export class AdminController {
  ...
  // Create user and email credentials (without changing AdminService.createUser)
  static async createUser(req: Request, res: Response) {
    try {
      const userData: CreateUserInput = req.body;
      console.log("Creating user with data:", userData);

      // 1) Create the user in DB (existing logic)
      const newUser = await AdminService.createUser(userData);
      console.log("User created successfully:", newUser);

      // 2) Send email with username + ORIGINAL password from request body
      // Do not block user creation if email fails.
      try {
        await sendAdminCreatedUserEmail({
          to: userData.email,
          username: userData.username,
          password: userData.password, // plain password only exists here
          firstName: userData.first_name,
          lastName: userData.last_name,
        });
        console.log("Credentials email sent to:", userData.email);
      } catch (mailErr) {
        console.error("Failed to send credentials email:", mailErr);
      }

      // 3) Return exactly what you returned before (to avoid breaking frontend expectations)
  ...
  static async getPosts(req: Request, res: Response) {
    try {
      ...
      const result = await AdminService.getAllPosts(parseInt(page as string), parseInt(limit as string));
  ...
  static async getEvents(req: Request, res: Response) {
    try {
      const { page = 1, limit = 10 } = req.query;
      const result = await AdminService.getAllEvents(parseInt(page as string), parseInt(limit as string));
