...
This project includes a fully functional Events module that allows admins to manage events and users to view public events.

### Database

The following tables are used:

- **events**: stores event information (title, slug, description, start/end times, timezone, location, capacity, visibility, status, created_by).
- **event_attendees**: tracks RSVPs and attendance for users and guests.
- **venues**: stores venue information.

The seed file (`src/seed.ts`) populates the database with:

- 3 sample venues
- 5 sample events
- Admin and Alumni users

Seed checks for existing users to avoid duplicates.
...
| GET | `/api/events` | List events with optional filters (status, date range, search text) | Public |
| GET | `/api/events/:id` | Get event details | Public |
| PATCH | `/api/events/:id` | Update core fields of an event | Admin only |
| DELETE | `/api/events/:id` | Soft delete an event | Admin only |

### Frontend Pages

- **Admin Event List**: table with search, sort, and pagination
- **Admin Event Form**: create and edit events
- **Public Event List**: card-based layout showing date, title, location, and tags
- **Public Event Details**: detailed view including description, schedule, capacity, and RSVP placeholder

### Running the Seed

The seed script will create default users, venues, and events if they do not already exist:
