...
class AuthService {
    ...
    async resetCheck(username: string, email: string): Promise<ResetCheckResponse> {
        try {
            const response = await api.post<ResetCheckResponse>('/auth/reset-check', { username, email });
    ...
    async resetConfirm(username: string, email: string, newPassword: string): Promise<ResetConfirmResponse> {
        try {
