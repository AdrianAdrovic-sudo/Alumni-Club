...
class AdminService {
    ...
    // User Management
    async getUsers(filters: UserFilters = {}, page: number = 1, limit: number = 10): Promise<PaginatedResponse<User>> {
    ...
    async deleteUser(userId: number): Promise<{ message: string }> {
        const response = await this.api.delete<{ message: string }>(`/admin/users/${userId}`);
        return response.data;
    }
    ...
    async getPosts(page: number = 1, limit: number = 10): Promise<PaginatedResponse<any>> {
        const response = await this.api.get<PaginatedResponse<any>>('/admin/posts', { params: { page, limit } });
        return response.data;
    }
    ...
    async getEvents(page: number = 1, limit: number = 10): Promise<PaginatedResponse<any>> {
        const response = await this.api.get<PaginatedResponse<any>>('/admin/events', { params: { page, limit } });
        return response.data;
    }
